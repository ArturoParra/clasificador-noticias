          ...
          <div className="space-y-8">
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-3">
                <Globe className={`size-6 ${isDarkMode ? 'text-blue-400' : 'text-blue-600'}`} />
                <h2 className={`text-2xl sm:text-3xl font-bold ${
                  isDarkMode ? 'text-white' : 'text-black'
                }`}>
                ...
                <span className={`text-sm px-3 py-1 rounded-full ${
                  ...
                  {customCarousels.length}/5 carruseles
                </span>
              </div>
              
              {customCarousels.length < 5 && (
                <button
                  onClick={() => setShowAddModal(true)}
                  className={`inline-flex items-center gap-2 px-4 py-2 rounded-xl font-medium transition-all duration-300 hover:scale-105 active:scale-95 ${
                    isDarkMode
            ...
            {/* Lista de carruseles personalizados */}
                  ...
                  >
                    <div className="flex items-center justify-between mb-4">
                      <div>
                        ...
                        <p className={`text-sm mt-1 ${
                          ...
                          {carousel.sources.length > 0 && `Fuentes: ${carousel.sources.join(', ')}`}
                          {carousel.sources.length > 0 && carousel.countries.length > 0 && ' • '}
                          {carousel.countries.length > 0 && `Países: ${carousel.countries.map(c => availableCountries.find(country => country.code === c)?.name || c).join(', ')}`}
                        </p>
                      </div>
                      <button
                    ...
                    <div className={`rounded-2xl overflow-hidden border ${
                      isDarkMode 
                        ? 'bg-gradient-to-br from-gray-900 to-black border-gray-800' 
                        : 'bg-gradient-to-br from-white to-gray-100 border-gray-300'
                    } p-4 sm:p-6 shadow-xl transition-all duration-500 relative`}>
                      ...
                      {filteredNews.length > 0 ? (
                        <>
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                            <AnimatePresence mode="wait">
                              {filteredNews.slice(currentIndex, currentIndex + 3).map((article, index) => (
                                <motion.div
                                  key={`${carousel.id}-${article.id}`}
                                  initial={{ opacity: 0, x: 50 }}
                                  animate={{ opacity: 1, x: 0 }}
                                  exit={{ opacity: 0, x: -50 }}
                                  transition={{ duration: 0.4, delay: index * 0.1 }}
                                  className={`rounded-xl overflow-hidden border transition-all duration-300 hover:scale-105 hover:shadow-2xl ${
                                    isDarkMode 
                          ...
                          {filteredNews.length > 3 && (
                            <div className="flex items-center justify-center gap-4 mt-6">
                              <button
                                onClick={() => prevCarousel(carousel.id)}
                                className={`p-2 rounded-full transition-all duration-300 hover:scale-110 active:scale-95 ${
                                  isDarkMode 
                                    ? 'bg-gray-800 hover:bg-gray-700 text-white' 
                                    : 'bg-gray-200 hover:bg-gray-300 text-black'
                              ...
                              <button
                                onClick={() => nextCarousel(carousel.id)}
                                className={`p-2 rounded-full transition-all duration-300 hover:scale-110 active:scale-95 ${
                                  isDarkMode 
                                    ? 'bg-gray-800 hover:bg-gray-700 text-white' 
                                    : 'bg-gray-200 hover:bg-gray-300 text-black'
            ...
            {customCarousels.length === 0 && (
              <div className={`rounded-2xl border-2 border-dashed p-12 text-center ${
                ...
                <Globe className={`size-16 mx-auto mb-4 ${
                  isDarkMode ? 'text-gray-700' : 'text-gray-400'
                }`} />
                <h3 className={`text-xl font-bold mb-2 ${
                  isDarkMode ? 'text-gray-400' : 'text-gray-600'
                }`}>
                  No hay carruseles personalizados
                </h3>
                <p className={`mb-6 ${
                  isDarkMode ? 'text-gray-600' : 'text-gray-500'
                }`}>
                  Crea tu primer carrusel para ver noticias de tus fuentes y países favoritos
                </p>
                <button
                  onClick={() => setShowAddModal(true)}
                  className={`inline-flex items-center gap-2 px-6 py-3 rounded-xl font-medium transition-all duration-300 hover:scale-105 active:scale-95 ${
                    isDarkMode
                      ? 'bg-blue-600 hover:bg-blue-700 text-white'
                      : 'bg-blue-500 hover:bg-blue-600 text-white'
                  }`}
